...
// Import data secara statis untuk dibundel oleh Vercel
import { 
  INITIAL_EMPLOYEES, 
  INITIAL_ATTENDANCE, 
...
let isDbInitialized = false; // Flag tambahan agar tidak connect berkali-kali

// Mock database fallback
const fallbackDb: any = {
  employees: INITIAL_EMPLOYEES.map(e => ({
    id: e.id, name: e.name, position: e.position, phone: e.phone, branch: e.branch,
    role: e.role, main_jobdesk: e.mainJobdesk, basic_salary: e.basicSalary,
    bonus: e.bonus, username: e.username, password_hash: e.passwordHash,
  })),
  attendance: INITIAL_ATTENDANCE.map(a => ({
    id: a.id, employee_id: a.employeeId, employee_name: a.employeeName, date: a.date,
    check_in_time: a.checkInTime, check_out_time: a.checkOutTime, status: a.status, keterangan: a.keterangan,
  })),
  daily_reports: INITIAL_DAILY_REPORTS.map(r => ({
    id: r.id, employee_id: r.employeeId, employee_name: r.employeeName, position: r.position,
    branch_name: r.branchName, shift: r.shift, date: r.date, daily_task: r.dailyTask, shift_revenue: r.shiftRevenue,
  })),
  shifts: INITIAL_SHIFTS.map(s => ({
    id: s.id, employee_id: s.employeeId, employee_name: s.employeeName, shift_type: s.shiftType, date: s.date,
  })),
  invoices: INITIAL_INVOICES.map(i => ({
    id: i.id, invoice_no: i.invoiceNo, branch_name: i.branchName, date: i.date, total: i.total, invoice_photo: i.invoicePhoto,
  })),
  payroll: INITIAL_PAYROLL.map(p => ({
    id: p.id, employee_id: p.employeeId, employee_name: p.employeeName, position: p.position,
    basic_salary: p.basicSalary, bonus: p.bonus, total_salary: p.totalSalary, period: p.period, date_paid: p.datePaid, status: p.status,
  }))
};
...
// --- INISIALISASI EXPRESS SECARA INSTAN (Wajib Sinkronus untuk Vercel) ---
const app = express();

app.use(express.json({ limit: '50mb' }));
app.use(express.urlencoded({ limit: '50mb', extended: true }));

// Middleware otomatis untuk memastikan koneksi DB dicek setiap ada request datang
...
// --- EXPORT SINKRONUS SANGAT PENTING UNTUK VERCEL FUNCTION ---
export default app;